...
static void
vehicleRenderCB(rw::Atomic *atomic, rw::gl1::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::gl1;
...
//#include "shaders/obj/neoVehicle_vert.inc"
	...
	rw::gl1::ObjPipeline *pipe = rw::gl1::ObjPipeline::create();
	pipe->instanceCB = rw::gl1::defaultInstanceCB;
...
DestroyVehiclePipe(void)
{
	...
	((rw::gl1::ObjPipeline*)vehiclePipe)->destroy();
	vehiclePipe = nil;
}
...
//rw::gl1::Shader *neoWorldShader;

static void
worldRenderCB(rw::Atomic *atomic, rw::gl1::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::gl1;

	if(!LightmapEnable){
		gl1::defaultRenderCB(atomic, header);
		return;
	}
...
DestroyWorldPipe(void)
{
	...
	((rw::gl1::ObjPipeline*)worldPipe)->destroy();
	worldPipe = nil;
}
...
rw::gl1::Shader *neoGlossShader;

static void
glossRenderCB(rw::Atomic *atomic, rw::gl1::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::gl1;
...
DestroyGlossPipe(void)
{
	...
	((rw::gl1::ObjPipeline*)glossPipe)->destroy();
	glossPipe = nil;
}
...
rw::gl1::Shader *neoRimShader;
rw::gl1::Shader *neoRimSkinShader;
...
static void
rimSkinRenderCB(rw::Atomic *atomic, rw::gl1::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::gl1;

	if(!RimlightEnable){
		gl1::skinRenderCB(atomic, header);
		return;
	}
...
static void
rimRenderCB(rw::Atomic *atomic, rw::gl1::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::gl1;
...
//	}
	...
	pipe = rw::gl1::ObjPipeline::create();
	pipe->instanceCB = rw::gl1::skinInstanceCB;
	pipe->uninstanceCB = nil;
...
DestroyRimLightPipes(void)
{
	...
	((rw::gl1::ObjPipeline*)rimPipe)->destroy();
	rimPipe = nil;

	((rw::gl1::ObjPipeline*)rimSkinPipe)->destroy();
	rimSkinPipe = nil;
}
...
{
	//u_viewVec = rw::gl1::registerUniform("u_viewVec");
	//u_rampStart = rw::gl1::registerUniform("u_rampStart");
	//u_rampEnd = rw::gl1::registerUniform("u_rampEnd");
	//u_rimData = rw::gl1::registerUniform("u_rimData");

	//u_lightMap = rw::gl1::registerUniform("u_lightMap");

	//u_eye = rw::gl1::registerUniform("u_eye");
	//u_reflProps = rw::gl1::registerUniform("u_reflProps");
	//u_specDir = rw::gl1::registerUniform("u_specDir");
	//u_specColor = rw::gl1::registerUniform("u_specColor");
}
...
{
	rw::Matrix matrix;
	rw::gl1::InstanceDataHeader *instHeader;
	uint32 cullMode;
	uint8 fadeAlpha;
	bool lighting;
};
...
IsTextureTransparent(RwTexture *tex)
{
	...
	//	return false;
	//return PLUGINOFFSET(rw::gl1::gl1Raster, tex->raster, rw::gl1::nativeRasterOffset)->hasAlpha;
...
AtomicFirstPass(RpAtomic *atomic, int pass)
{
	...
	building->instHeader = (gl1::InstanceDataHeader*)atomic->geometry->instData;
	assert(building->instHeader != nil);
	assert(building->instHeader->platform == PLATFORM_GL1);
