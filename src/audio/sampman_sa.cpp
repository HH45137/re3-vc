...
namespace SteamAudio 
{
	...
	bool InitSteamAudio()
	{
		...
		if (iplContextCreate(&contextSettings, &SteamAudio::context) != IPL_STATUS_SUCCESS) {
...
{26, 1.7f, 0.8f, -1000, -1000, -100, 4.42f, 0.14f, 1.00f, 429, 0.014f, 0.00f, 0.00f, 0.00f, 1023, 0.021f, 0.00f, 0.00f, 0.00f, 0.250f, 0.000f, 0.250f,
 0.000f, -5.0f, 2727.1f, 250.0f, 0.00f, 0x3f};

EAXLISTENERPROPERTIES FinishEAX3 =
{26, 100.0f, 1.0f, 0, -1000, -2200, 20.0f, 1.39f, 1.00f, 1000, 0.069f, 0.00f, 0.00f, 0.00f, 400, 0.100f, 0.00f, 0.00f, 0.00f, 0.250f, 1.000f, 3.982f,
 0.000f, -18.0f, 3530.8f, 417.9f, 6.70f, 0x3f};
...
IsFXSupported()
{
	return _effectsSupported; // usingEAX || usingEAX3 || _usingEFX || usingSteamAudio;
}

void
EAX_SetAll(const EAXLISTENERPROPERTIES *allparameters)
{
	if (SteamAudio::usingSteamAudio)
	{
		
		return;
	}
	
	if(usingEAX || usingEAX3) EAX3_Set(ALEffect, allparameters);
	else EFX_Set(ALEffect, allparameters);
}
...
add_providers()
{
	...
	if((pDeviceList) && (pDeviceList->GetNumDevices())) {
		...
		for(int j = n; j < MAXPROVIDERS; j++) SampleManager.Set3DProviderName(j, NULL);
...
{
	if(IsFXSupported()) {
		if(alIsEffect(ALEffect)) { alEffecti(ALEffect, AL_EFFECT_TYPE, AL_EFFECT_NULL); }

		if(alIsAuxiliaryEffectSlot(ALEffectSlot)) { alAuxiliaryEffectSloti(ALEffectSlot, AL_EFFECTSLOT_EFFECT, AL_EFFECT_NULL); }
	}
...
{
	if(curprovider == index) return TRUE;
	...
	if(curprovider != -1) {
		...
		if(IsFXSupported()) { for(int32 i = 0; i < MAXCHANNELS; i++) aChannel[i].SetReverbMix(ALEffectSlot, 0.0f); }
...
static bool8
IsThisTrackAt16KHz(uint32 track) { return track == STREAMED_SOUND_RADIO_KCHAT || track == STREAMED_SOUND_RADIO_VCPR || track == STREAMED_SOUND_RADIO_POLICE; }
...
{
	if(_maxSamples > MAXCHANNELS) return MAXCHANNELS;

	return _maxSamples;
}

uint32
cSampleManager::GetNum3DProvidersAvailable() { return m_nNumberOfProviders; }

void
cSampleManager::SetNum3DProvidersAvailable(uint32 num) { m_nNumberOfProviders = num; }

char *
cSampleManager::Get3DProviderName(uint8 id) { return m_aAudioProviders[id]; }

void
cSampleManager::Set3DProviderName(uint8 id, char *name) { m_aAudioProviders[id] = name; }

int8
cSampleManager::GetCurrent3DProviderIndex(void) { return curprovider; }
...
cSampleManager::SetCurrent3DProvider(uint8 nProvider)
{
	...
	if(set_new_provider(nProvider)) return curprovider;
	else if(savedprovider != -1 && savedprovider < m_nNumberOfProviders && set_new_provider(savedprovider)) return curprovider;
	else return curprovider;
}

int8
cSampleManager::AutoDetect3DProviders()
{
	if(!AudioManager.IsAudioInitialised()) return -1;

	if(defaultProvider >= 0 && defaultProvider < m_nNumberOfProviders) { if(set_new_provider(defaultProvider)) return defaultProvider; }
	...
	for(uint32 i = 0; i < GetNum3DProvidersAvailable(); i++) {
		...
		if(!strcasecmp(providername, "OPENAL SOFT")) {
			...
			if(GetCurrent3DProviderIndex() == i) return i;
		}
	}
...
_ResolveLink(char const *path, char *out)
{
	...
	if(len < 4 || strcmp(&path[len - 4], ".lnk") != 0) return FALSE;
		...
		if(bInitFirstEntry) {
			if(filepathlen > 0) {
				if(_ResolveLink(filepath, filepath)) {
					...
					bShortcut = FALSE;
					if(filepathlen > MAX_PATH) { continue; }
				}
...
_GetMP3EntryByIndex(uint32 idx)
{
	...
	if(_pMP3List != NULL) {
		...
		for(uint32 i = 0; i < n; i++) e = e->pNext;
...
cSampleManager::Initialise(void)
{
	...
	for(int i = 0; i < MAX_STREAMS; i++) aStream[i] = new CStream(ALStreamSources[i], ALStreamBuffers[i]);
	...
	{
		for(int32 i = 0; i < NUM_CHANNELS; i++) nChannelVolume[i] = 0;
	}
...

	{
		...
		for(int32 i = 0; i < MAXCHANNELS; i++) aChannel[i].Init(i);
		for(int32 i = 0; i < MAX2DCHANNELS; i++) aChannel[MAXCHANNELS + i].Init(MAXCHANNELS + i, true);
		...
		if(IsFXSupported()) {
			...
			for(int32 i = 0; i < MAXCHANNELS; i++) aChannel[i].SetReverbMix(ALEffectSlot, 0.0f);
		}
	}

	{
		for(int32 i = 0; i < TOTAL_STREAMED_SOUNDS; i++) nStreamLength[i] = 0;
	}
	...
	if(cacheFile) {
		...
		if(cacheFile) {
			...
			fclose(cacheFile);
		} else { debug("Cannot save audio cache\n"); }
	}
#endif

	{
		...
		for(uint32 i = PLAYER_COMMENTS_START; i <= PLAYER_COMMENTS_END; i++) nMaxPedSize = Max(nMaxPedSize, m_aSamples[i].nSize);
...

	{
		...
		if(defaultProvider >= 0 && defaultProvider < m_nNumberOfProviders) { set_new_provider(defaultProvider); } else {
			Terminate();
			return FALSE;
		}
	}

	{
		...
		if(nNumMP3s != 0) {
			...
			int32 randval;
			if(bUseRandomTable) randval = AudioManager.m_anRandomTable[1];
			else randval = localtm->tm_sec * localtm->tm_min;

			_CurMP3Index = randval % nNumMP3s;

			tMP3Entry *randmp3 = _pMP3List;
			for(int32 i = randval % nNumMP3s; i > 0; --i) randmp3 = randmp3->pNext;

			if(bUseRandomTable) _CurMP3Pos = AudioManager.m_anRandomTable[0] % randmp3->nTrackLength;
			else {
				if(localtm->tm_sec > 0) {
	...
	SteamAudio::usingSteamAudio = SteamAudio::InitSteamAudio();
...
{
	for(int32 i = 0; i < MAX_STREAMS; i++) aStream[i]->Close();

	for(int32 i = 0; i < NUM_CHANNELS; i++) aChannel[i].Term();
	...
	for(int32 i = 0; i < MAX_STREAMS; i++) { alDeleteBuffers(NUM_STREAMBUFFERS, ALStreamBuffers[i]); }
	...
	for(int32 i = 0; i < MAX_STREAMS; i++) delete aStream[i];
	...
	if(_bSampmanInitialised) {
		for(int32 i = 0; i < NUM_CHANNELS; i++) {
			if(GetChannelUsedFlag(i)) {
				if(nChannelVolume[i] != 0) aChannel[i].SetVolume(m_nEffectsFadeVolume * nChannelVolume[i] * m_nEffectsVolume >> 14);
			}
		}
	}
}
...
cSampleManager::LoadMissionAudio(uint8 nSlot, uint32 nSample)
{
	...
	if(fseek(fpSampleDataHandle, m_aSamples[nSample].nOffset, SEEK_SET) != 0) return FALSE;

	if(fread(gPlayerTalkData, 1, m_aSamples[nSample].nSize, fpSampleDataHandle) != m_aSamples[nSample].nSize) return FALSE;
...
#endif
		if(nComment == nPedSlotSfx[slot]) return LOADING_STATUS_LOADED;
	}
...
cSampleManager::LoadPedComment(uint32 nComment)
{
	...
	while(samplesSize > 0) {
		int size = op_read(fpSampleDataHandle,
		                   (opus_int16 *)(nSampleBankMemoryStartAddress[SAMPLEBANK_PED] + PED_BLOCKSIZE * nCurrentPedSlot + samplesRead),
		                   samplesSize, NULL);
...
{
	if(!usingEAX && !_usingEFX) return FALSE;

	if(AudioManager.m_FrameCounter & 15) return FALSE;
...
#define CALCULATE_RATIO(value, maxDist, maxRatio) (value > MIN_DIST && value < maxDist ? value / maxDist * maxRatio : 0)
	...
	fRatio += CALCULATE_RATIO((AudioManager.m_afReflectionsDistances[REFLECTION_NORTH] + AudioManager.m_afReflectionsDistances[REFLECTION_SOUTH]) / 2.f,
	                          4.0f, 1/3.f);
	fRatio += CALCULATE_RATIO((AudioManager.m_afReflectionsDistances[REFLECTION_WEST] + AudioManager.m_afReflectionsDistances[REFLECTION_EAST]) / 2.f, 4.0f,
	                          1/3.f);
...
#undef MIN_DIST
	...
	if(fRatio == _fPrevEaxRatioDestination) return FALSE;
...
#endif
	{
		...
		if(_usingEFX) _fEffectsLevel = fRatio * 0.8f;
		else _fEffectsLevel = fRatio * 0.22f;
	}
...
cSampleManager::SetChannelReverbFlag(uint32 nChannel, bool8 nReverbFlag)
{
	...
	if(usingEAX || _usingEFX) {
		if(IsFXSupported()) {
			...
			if(nReverbFlag != FALSE) aChannel[nChannel].SetReverbMix(ALEffectSlot, _fEffectsLevel);
			else aChannel[nChannel].SetReverbMix(ALEffectSlot, 0.0f);
		}
	}
}
...
cSampleManager::SetChannelPan(uint32 nChannel, uint32 nPan)
{
	...
	if(nChannel == CHANNEL_POLICE_RADIO) { aChannel[nChannel].SetPan(nPan); }
}
...
cSampleManager::PauseStream(bool8 nPauseFlag, uint8 nStream)
{
	...
	if(stream->IsOpened()) { stream->SetPause(nPauseFlag != FALSE); }
}
...
cSampleManager::StartStreamedFile(uint32 nFile, uint32 nPos, uint8 nStream)
{
	...
	if(nFile >= TOTAL_STREAMED_SOUNDS) return FALSE;
			...
			if(!_bIsMp3Active && i == 0) {
				...
				if(!_GetMP3PosFromStreamPos(&position, &e) && !e) {
					...
					if(stream->Setup()) {
						...
						if(position != 0) stream->SetPosMS(position);
					...
					if(e->pLinkPath != NULL) aStream[nStream]->Open(e->pLinkPath, IsThisTrackAt16KHz(nFile) ? 16000 : 32000);
					else {
				...
				if(mp3->pLinkPath != NULL) aStream[nStream]->Open(mp3->pLinkPath, IsThisTrackAt16KHz(nFile) ? 16000 : 32000);
				else {
					strcpy(filename, _mp3DirectoryPath);
...
cSampleManager::StopStreamedFile(uint8 nStream)
{
	...
	if(nStream == 0) _bIsMp3Active = FALSE;
}
...
cSampleManager::SetStreamedVolumeAndPan(uint8 nVolume, uint8 nPan, bool8 nEffectFlag, uint8 nStream)
{
	...
	if(MusicManager.GetRadioInCar() == USERTRACK && !MusicManager.CheckForMusicInterruptions()) boostMult = m_nMP3BoostVolume / 64.f;
	...
	if(stream->IsOpened()) {
		if(nEffectFlag) {
			if(nStream == 1 || nStream == 2) stream->SetVolume(128 * nVolume * m_nEffectsVolume >> 14);
			else stream->SetVolume(m_nEffectsFadeVolume * nVolume * m_nEffectsVolume >> 14);
		} else stream->SetVolume((m_nMusicFadeVolume * nVolume * (uint32)(m_nMusicVolume * boostMult + m_nMusicVolume)) >> 14);
...
cSampleManager::IsStreamPlaying(uint8 nStream)
{
	...
	if(stream->IsOpened()) { if(stream->IsPlaying()) return TRUE; }
...
cSampleManager::Service(void)
{
	for(int32 i = 0; i < MAX_STREAMS; i++) {
		...
		if(stream->IsOpened()) stream->Update();
	}
	int refCount = CChannel::channelsThatNeedService;
	for(int32 i = 0; refCount && i < NUM_CHANNELS; i++) { if(aChannel[i].Update()) refCount--; }
}
...
cSampleManager::InitialiseSampleBanks(void)
{
	...
	if(fpSampleDescHandle == NULL) return FALSE;
#ifndef OPUS_SFX
...
void
cSampleManager::SetStreamedFileLoopFlag(bool8 nLoopFlag, uint8 nChannel) { nStreamLoopedFlag[nChannel] = nLoopFlag; }
